...
#define SICHERHEITSROLLPICH 45  // Sicherheitdbegrenzung bei der Messung der Wasssertiefe. Ab diesem Wert bei Roll und Pich setzt das Echolot aus.
...
void echolot_lesen() {
  ...
  if (avg_dist > 0) {
    // Sicherheitsgrenze IMU
    if (fabs(sensorData.roll) > SICHERHEITSROLLPICH || fabs(sensorData.pitch) > SICHERHEITSROLLPICH) {
      return;
    }

    // Roll + Pitch Korrektur
    double roll_rad = sensorData.roll * DEG_TO_RAD;
    double pitch_rad = sensorData.pitch * DEG_TO_RAD;

    // Vertikale Tiefe berechnen
    double dist_vertical = avg_dist * cos(roll_rad) * cos(pitch_rad);
