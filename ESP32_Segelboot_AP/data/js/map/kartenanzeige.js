...
/* ==========================================================
   NAVIS PANEL MANAGER
   Ordnet alle minimierten Panels automatisch an.
========================================================== */
function layoutCollapsedPanels() {
    const panels = document.querySelectorAll(".navis-panel");
    let bottom = 20;
    ...
    panels.forEach(panel => {
        ...
        panel.style.right = PANEL_RIGHT + "px";
...
map.on('click', e => {
  ...
  if (currentMode === "marker") {
    ...
    // ÄNDERUNG: Popup-Konfiguration angepasst, um das Schließkreuz (X) zu erzwingen
    L.popup({
      closeButton: true,
      autoClose: true,
      closeOnClick: true
  ...
  if (currentMode === "bearing") {
    ...
    const { bearing, distance } = bearingAndDistance(boatLat, boatLon, e.latlng.lat, e.latlng.lng);
...
window.addEventListener("navisTelemetryUpdate", (event) => {
    const j = event.detail.raw;
    if ("gps_lat" in j)
        target.gps_lat = Number(j.gps_lat);
    if ("gps_lon" in j)
        target.gps_lon = Number(j.gps_lon);
    if ("gps_speed" in j)
        target.gps_speed = Number(j.gps_speed);
    if ("gps_kurs" in j)
        target.gps_kurs = Number(j.gps_kurs);
    if ("sm" in j) {
        distanceSM = Number(j.sm);
        updateLogDisplay();
    }
    if ("kompass" in j)
		if (target.kompass !== Number(j.kompass))
			target.kompass = Number(j.kompass);
    if ("winddir_berechnet" in j)
		if (target.winddir_berechnet !== Number(j.winddir_berechnet))
			target.winddir_berechnet = Number(j.winddir_berechnet);
    if ("windspeed_berechnet" in j)
		if (target.windspeed_berechnet !==  Number(j.windspeed_berechnet))
			target.windspeed_berechnet = Number(j.windspeed_berechnet);
});
...
const FOLLOW_INTERVAL = 10000; // ms
let lastLat = null;
let lastLon = null;
let lastHeading = null;
const GPS_EPSILON = 0.000001;   // ca. 11 cm
const HDG_EPSILON = 0.2;        // 0,2°

function updateBoatGeometry() {
	if (
		Math.abs(state.gps_lat - lastLat) < GPS_EPSILON &&
		Math.abs(state.gps_lon - lastLon) < GPS_EPSILON &&
		Math.abs(angleDiff(lastHeading, state.kompass)) < HDG_EPSILON
	) {
		return;
	}

    lastLat = state.gps_lat;
    lastLon = state.gps_lon;
    lastHeading = state.kompass;
...
function updateLegend() {
    // Hole die Übersetzungen direkt aus dem geladenen Sprachobjekt (mit deutschem Fallback)
    const txtCompass = getTranslation("map_leg_compass", "🔴 Kompasskurs:");
    const txtGps = getTranslation("map_leg_gps", "🟡 GPS-Kurs:");
    const txtWind = getTranslation("map_leg_wind", "🟢 Wind:");

    // Baue die Legende dynamisch und übersetzungssicher zusammen
    legendEl.innerHTML =
        `${txtCompass} ${state.kompass.toFixed(1)}°<br>` +
        `${txtGps} ${state.gps_kurs.toFixed(1)}°<br>` +
        `${txtWind} ${state.winddir_berechnet.toFixed(1)}° / ${state.windspeed_berechnet.toFixed(1)} kn`;
}
...
function jumpToWeatherFrame(hour) {
    if (hour === undefined || hour === null) return;
    const slider = document.getElementById("weather-slider");
    if (!slider) return;
    slider.value = hour;
    // Wetterdarstellung aktualisieren
    updateWeatherFrame(hour);
}

// =========================================================================
// GLOBALE ROUTEN-KONFIGURATION
// =========================================================================
const NAVIS_CONFIG = {
    MANEUVER_THRESHOLD: 20.0,      // Kursänderung in Grad, ab der ein WP erzeugt wird
...
function drawCalculatedRoute(route) {
	...
	// =========================================================================
	// 3. INTELLIGENTER GERADENFILTER V2
	// =========================================================================
	const filteredRoute = [];
	filteredRoute.push(route[0]);
	let anchor = 0;
	const lastIndex = route.length - 1;
	while (anchor < lastIndex - 1) {
		let lastGood = anchor + 1;
		for (let test = anchor + 2; test < lastIndex; test++) {
			const start = route[anchor];
			const end = route[test];
			let keepSegment = true;
			const segmentBearing = calculateBearing(start.lat, start.lon, end.lat, end.lon);
			for (let i = anchor + 1; i < test; i++) {
				const prev = route[i - 1];
				const p = route[i];
				const next = route[i + 1];
				// 1. Kurs auf Geraden
				const bearing = calculateBearing(start.lat, start.lon, p.lat, p.lon);
				let delta = Math.abs(segmentBearing - bearing);
				if (delta > 180) delta = 360 - delta;
				// 2. Seitliche Abweichung
				const direct = calculateDistance(start.lat, start.lon, end.lat, end.lon);
				const via = calculateDistance(start.lat, start.lon, p.lat, p.lon) + calculateDistance(p.lat, p.lon, end.lat, end.lon);
				const deviation = via - direct;
				// 3. Echter Kurswechsel am Wegpunkt?
				const b1 = calculateBearing(prev.lat, prev.lon, p.lat, p.lon);
				const b2 = calculateBearing(p.lat, p.lon, next.lat, next.lon);
				let maneuver = Math.abs(b2 - b1);
				if (maneuver > 180) maneuver = 360 - maneuver;
				if (delta > NAVIS_CONFIG.MANEUVER_THRESHOLD || deviation > 0.03 || maneuver > NAVIS_CONFIG.MANEUVER_THRESHOLD) {
					keepSegment = false;
					break;
				}
			}
			if (keepSegment) lastGood = test;
			else break;
		}
		filteredRoute.push(route[lastGood]);
		anchor = lastGood;
	}
	// Vorletzten Punkt nur übernehmen, wenn dort ein echter Kurswechsel beginnt
	if (route.length >= 3) {
		const a = route[lastIndex - 2];
		const b = route[lastIndex - 1];
		const c = route[lastIndex];
		const b1 = calculateBearing(a.lat, a.lon, b.lat, b.lon);
		const b2 = calculateBearing(b.lat, b.lon, c.lat, c.lon);
		let delta = Math.abs(b2 - b1);
		if (delta > 180) delta = 360 - delta;
		if (delta >= NAVIS_CONFIG.MANEUVER_THRESHOLD) {
			if (filteredRoute[filteredRoute.length - 1] !== b) filteredRoute.push(b);
		}
	}
	// Ziel immer übernehmen
	if (filteredRoute[filteredRoute.length - 1] !== route[lastIndex]) {
		filteredRoute.push(route[lastIndex]);
	}
	// NEU: ZIELGEBIET-FILTER (Löscht den vorletzten Punkt nur, wenn dort KEINE ordentliche Kursänderung vorliegt)
	if (NAVIS_CONFIG.FILTER_DESTINATION_KINK && filteredRoute.length >= 3) {
		...
		const thirdLastPt = filteredRoute[filteredRoute.length - 3];
		const distanceMeters = L.latLng(targetPt.lat, targetPt.lon).distanceTo(L.latLng(secondLastPt.lat, secondLastPt.lon));
		const distanceNM = distanceMeters / 1852;
		if (distanceNM <= 0.2) {
			const b1 = calculateBearing(thirdLastPt.lat, thirdLastPt.lon, secondLastPt.lat, secondLastPt.lon);
			const b2 = calculateBearing(secondLastPt.lat, secondLastPt.lon, targetPt.lat, targetPt.lon);
			let maneuver = Math.abs(b2 - b1);
			if (maneuver > 180) maneuver = 360 - maneuver;
			if (maneuver < NAVIS_CONFIG.MANEUVER_THRESHOLD) {
    ...
    filteredRoute.forEach((p, idx) => {
		...
		const etaInfo = formatNavisETA(p);

        // Tooltip mit Navigationsdaten
		const tooltipText = `
		<b>🧭 WP #${idx + 1}</b><br>
		⏱ Reisezeit: ${etaInfo.travel}<br>
		🕒 ETA UTC: ${etaInfo.utc}<br>
		🕒 ETA Lokal: ${etaInfo.local}<br>
		🗓 Wetter-Frame: +${p.forecastHour ?? 0} h<br>
		...
		marker.on('click', () => {
			const hour = p.forecastHour ?? p.weatherFrame ?? p.etaHour ?? 0;
			jumpToWeatherFrame(hour);
		});
